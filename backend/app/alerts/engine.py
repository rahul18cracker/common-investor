...
def snapshot_price_for_ticker(ticker: str):
    ...
    cid = int(row[0])
    execute(
        "INSERT INTO price_snapshot (company_id, price, source, currency) VALUES (:cid,:p,:src,:ccy)",
        cid=cid,
        p=p,
        src="yfinance",
        ccy="USD",
...
def evaluate_alerts():
    rules = execute(
        ...
        "FROM alert_rule ar JOIN company c ON c.id=ar.company_id WHERE ar.enabled"
    ).fetchall()
    triggered = []
    ...
    for rid, ticker, p, typ in triggered:
        ...
        """,
            text=f"ALERT {typ}: price {p} triggered",
            typ=typ,
            t=ticker,
        )

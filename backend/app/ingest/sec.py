...
BS_TAGS = {
    ...
    "shareholder_equity": ["StockholdersEquityIncludingPortionAttributableToNoncontrollingInterest", "StockholdersEquity"]
}
CF_TAGS = {
...
def _annual_value(units: dict, unit_key: str, fy: int) -> float|None:
    if not units or unit_key not in units: return None
    best = None
    for item in units[unit_key]:
        if item.get("fy") != fy: continue
        if item.get("form") not in ("10-K","20-F"): continue
        v = item.get("val")
        if isinstance(v, (int,float)): best = float(v)
    return best

def ingest_companyfacts_richer_by_ticker(ticker: str):
    t2c = ticker_map()
    cik = t2c.get(ticker.upper())
    if not cik: raise ValueError(f"No CIK found for {ticker}")
    facts = company_facts(cik)
    entity = facts.get("entityName", "")
    upsert_company(f"{cik:010d}", ticker.upper(), name=entity)

    units_cache = {"is": {}, "bs": {}, "cf": {}}
    for k, tags in IS_TAGS.items(): units_cache["is"][k] = _pick_first_units(facts, tags)
    for k, tags in BS_TAGS.items(): units_cache["bs"][k] = _pick_first_units(facts, tags)
    for k, tags in CF_TAGS.items(): units_cache["cf"][k] = _pick_first_units(facts, tags)

    years = set()
    for units, key in ((units_cache["is"]["revenue"],"USD"), (units_cache["is"]["eps_diluted"],"USD/shares")):
        if units and key in units:
            for item in units[key]:
                fy=item.get("fy"); form=item.get("form","")
                if fy and form in ("10-K","20-F"): years.add(int(fy))
    years = sorted(years)

    for fy in years:
        filing_id = upsert_filing(cik=f"{cik:010d}", form="10-K", accession=f"FACTS-{cik}-{fy}", period_end=f"{fy}-12-31")
        # IS
        execute("""
        ...
        """, filing_id=filing_id, fy=fy,
             revenue=_annual_value(units_cache["is"]["revenue"], "USD", fy),
             ebit=_annual_value(units_cache["is"]["ebit"], "USD", fy),
             interest_expense=_annual_value(units_cache["is"]["interest_expense"], "USD", fy),
             taxes=_annual_value(units_cache["is"]["taxes"], "USD", fy),
             net_income=_annual_value(units_cache["is"]["net_income"], "USD", fy),
             eps_diluted=_annual_value(units_cache["is"]["eps_diluted"], "USD/shares", fy),
             shares_diluted=_annual_value(units_cache["is"]["shares_diluted"], "shares", fy))
        # BS
        execute("""
        ...
        """, filing_id=filing_id, fy=fy,
             ...
             total_liabilities=_annual_value(units_cache["bs"]["total_liabilities"], "USD", fy),
             total_debt=_annual_value(units_cache["bs"]["total_debt"], "USD", fy),
             shareholder_equity=_annual_value(units_cache["bs"]["shareholder_equity"], "USD", fy))
        # CF
        execute("""

...
def _fetch_is_series(cik: str) -> List[Tuple[int, Optional[float], Optional[float], Optional[float]]]:
    rows = execute("""
...
def compute_growth_metrics(cik: str) -> Dict[str, Optional[float]]:
    ...
    if not series or len(series) < 2:
        return {"rev_cagr_5y": None, "rev_cagr_10y": None, "eps_cagr_5y": None, "eps_cagr_10y": None}
    ...
    def window(vals, n):
        ...
        first_idx = next((i for i,(fy,v) in enumerate(zip(years, vals)) if fy >= first_year and v is not None), None)
        last_idx = next((i for i,(fy,v) in reversed(list(enumerate(zip(years, vals)))) if fy <= last_year and v is not None), None)
        if first_idx is None or last_idx is None or last_idx <= first_idx:
            return None
        span_years = years[last_idx] - years[first_idx]
        if span_years <= 0: return None
...
def roic_series(cik: str):
    ...
    out = []
    for r in rows:
        fy, ebit, taxes, debt, equity, cash = r["fy"], r["ebit"], r["taxes"], r["debt"], r["equity"], r["cash"]
        if fy is None: continue
        roic = None
...
def revenue_eps_series(cik: str):
    rows = execute("""
        ...
        ORDER BY si.fy ASC
    """, cik=cik).fetchall()
    return [{"fy": int(fy), "revenue": float(rev) if rev is not None else None, "eps": float(eps) if eps is not None else None}

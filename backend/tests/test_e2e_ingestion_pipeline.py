...
class TestEndToEndIngestionPipeline:
    ...
    def test_complete_ingestion_flow(
        self, client, db_session, celery_worker, mock_httpx_client, mock_sec_company_facts
    ):
        """
    ...
    def test_ingestion_idempotency(self, client, db_session, celery_worker, httpx_mock):
        """
    ...
    def test_parallel_ingestions(self, client, db_session, celery_worker, httpx_mock):
        """
        ...
        for _ in tickers:
            httpx_mock.add_response(url="https://www.sec.gov/files/company_tickers.json", json=ticker_data)
...
class TestDataPersistence:
    ...
    def test_financial_statements_persistence(
        ...
        statements = (
            db_session.query(StatementIS).filter(StatementIS.filing_id.in_(filing_ids)).all() if filing_ids else []
        )
    ...
    def test_company_metadata_persistence(self, client, db_session, celery_worker, mock_httpx_client):
        """
    ...
    def test_filing_dates_persistence(self, client, db_session, celery_worker, mock_httpx_client):
        """
...
class TestAPIResponseIntegrity:
    ...
    def test_get_company_response_structure(self, client, celery_worker, mock_httpx_client):
        """
    ...
    def test_metrics_endpoint_after_ingestion(self, client, celery_worker, mock_httpx_client):
        """
...
class TestIngestionErrorHandling:
    ...
    def test_invalid_ticker_ingestion(self, client, celery_worker, httpx_mock, db_session):
        """
    ...
    def test_sec_api_failure_handling(self, client, celery_worker, httpx_mock, db_session):
        """
...
class TestIngestionPerformance:
    ...
    def test_single_ingestion_performance(self, client, celery_worker, mock_httpx_client):
        """
    ...
    def test_batch_ingestion_performance(self, client, celery_worker, mock_httpx_client, mock_sec_company_facts):
        """
        ...
        start_time = time_module_2.time()

        response = client.post(f"/api/v1/company/{ticker}/ingest")
        assert response.status_code == 200

        duration = time_module_2.time() - start_time
...
class TestDataConsistency:
    ...
    def test_referential_integrity(self, client, db_session, celery_worker, mock_httpx_client):
        """

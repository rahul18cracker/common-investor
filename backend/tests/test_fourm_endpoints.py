...
def test_fourm_analysis_endpoint_success():
    ...
    with patch('app.api.v1.routes.execute') as mock_execute, \
        ...
        mock_execute.return_value.first.return_value = ["0000123456"]
...
def test_meaning_refresh_endpoint_success():
    ...
    with patch('app.api.v1.routes.execute') as mock_execute, \
        ...
        mock_execute.return_value.first.side_effect = [
            ["0000123456"],  # Company lookup
            None,  # No existing meaning note
        ]
...
def test_meaning_refresh_endpoint_existing_recent_note():
    ...
    with patch('app.api.v1.routes.execute') as mock_execute, \
        ...
        # Verify that INSERT was not called (only 2 execute calls: company lookup + existing check)

...
export default function CompanyPage({ ticker }: { ticker: string }) {
  ...
  const loadAll = async (): Promise<void> => {
    setLoading(true);
    try {
      const s = await fetch(`${api}/api/v1/company/${ticker}`);
  ...
  const handleIngest = async (): Promise<void> => {
    setIngesting(true);
    try {
      await fetch(`${api}/api/v1/company/${ticker}/ingest`, { method: 'POST' });
  ...
  useEffect(() => { loadAll(); }, [ticker, api]);

  const metricsCSV = `${api}/api/v1/company/${ticker}/export/metrics.csv`;
  ...
  return (
    <main style={{ padding: '24px 24px 48px', maxWidth: 1200, margin: '0 auto' }}>
      ...
      {!loading && !summary && (
        <div style={{ padding: 20, background: '#fffbeb', borderRadius: 8, marginBottom: 24 }}>
          <p style={{ margin: 0, color: '#b45309' }}>
            No data available for {ticker}. Click &quot;Ingest SEC Data&quot; to fetch financial information from SEC EDGAR.
          </p>
        </div>
      )}
      ...
      <BigFivePanel api={api} ticker={ticker} />
      <IncomeStatementPanel api={api} ticker={ticker} />
      <FourMsPanel api={api} ticker={ticker} />
      <ValuationPanel api={api} ticker={ticker} />
      <CompanyDashboard api={api} ticker={ticker} />
      <AlertsPanel api={api} ticker={ticker} />
    </main>
  );
}
